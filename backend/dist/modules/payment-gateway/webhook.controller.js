...
class WebhookController {
    ...
    get eventEmitter() {
        if (!this._eventEmitter) {
            this._eventEmitter = (0, EventEmitterService_1.getEventEmitterService)(this.pool);
        }
      ...
      `;
            ...
            try {
                if (event === 'PAYMENT_CREATED' || payment.status === 'PENDING') {
                    ...
                    await this.eventEmitter.emitPaymentOverdue(tenantId, charge.id, {
                        chargeId: charge.id,
                        gatewayChargeId: payment.id,
                        gateway: 'asaas',
                        value: payment.value,
     ...
     */
    async processPagBankWebhook(webhookId, tenantId, payload) {
        try {
            // Update webhook status
            await this.pool.query(`UPDATE payment_webhooks SET status = 'processing', "updatedAt" = NOW() WHERE id = $1`, [webhookId]);
            const event = payload.event;
      ...
      `;
            ...
            const chargeResult = await this.pool.query(chargeQuery, [
                ...
                null, // invoiceUrl
                charge.links?.find((l) => l.media === 'image/png')?.href || null, // PIX QR Code
                charge.links?.find((l) => l.rel === 'QRCODE')?.href || null, // PIX copy/paste
