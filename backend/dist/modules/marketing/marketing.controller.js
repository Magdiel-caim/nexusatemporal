...
class MarketingController {
    ...
    async listAIConfigs(req, res) {
        try {
            ...
            const aiConfigService = (await Promise.resolve().then(() => __importStar(require('./ai-config.service')))).default.getInstance();
    ...
    // ============================================
    async generateAICopyV2(req, res) {
        try {
            const tenantId = req.user?.tenantId;
            const userId = req.user?.userId;
            if (!tenantId) {
            ...
            const result = await this.aiAssistantService.generateCopy(tenantId, provider, prompt, context, userId ? parseInt(userId) : undefined);
    ...
    async generateSummary(req, res) {
        try {
            ...
            const result = await this.aiAssistantService.generateSummary(tenantId, provider, text, maxLength || 200, userId ? parseInt(userId) : undefined);
    ...
    async translateText(req, res) {
        try {
            ...
            const result = await this.aiAssistantService.translateText(tenantId, provider, text, targetLanguage, userId ? parseInt(userId) : undefined);
    ...
    async getAIUsageStats(req, res) {
        try {
            ...
            const aiProviderService = (await Promise.resolve().then(() => __importStar(require('./ai-provider.service')))).default.getInstance();
    ...
    async updateAIRateLimits(req, res) {
        try {
            const tenantId = req.user?.tenantId;
            if (!tenantId) {
                res.status(401).json({ success: false, message: 'Unauthorized' });
            ...
            const { maxRequestsPerHour, maxTokensPerDay, maxCostPerMonthUsd, alertsEnabled, alertThresholdPercent, } = req.body;
            const aiProviderService = (await Promise.resolve().then(() => __importStar(require('./ai-provider.service')))).default.getInstance();
            const db = aiProviderService['db'];
            const query = `
