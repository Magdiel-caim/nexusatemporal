...
const comissao_service_1 = require("./comissao.service");
class VendasController {
    vendasService;
    comissaoService;
    constructor(db) {
        this.vendasService = new vendas_service_1.VendasService(db);
        this.comissaoService = new comissao_service_1.ComissaoService(db);
    }
    // ============================================
    // VENDEDORES
    // ============================================
     ...
     */
    getVendasByVendedor = async (req, res) => {
        try {
            const { id } = req.params;
            const { tenantId } = req.user;
     ...
     */
    getVendasStats = async (req, res) => {
        try {
            const { tenantId } = req.user;
            const { vendedorId } = req.query;
            const stats = await this.vendasService.getVendasStats(tenantId, vendedorId);
            res.json(stats);
        }
        catch (error) {
     ...
     */
    pagarComissao = async (req, res) => {
        try {
            const { id } = req.params;
            const { transactionId } = req.body;
            const comissao = await this.comissaoService.marcarComoPaga(id, transactionId);
            res.json(comissao);
        }
        catch (error) {
     ...
     */
    relatorioComissoes = async (req, res) => {
        try {
            const { tenantId } = req.user;
     ...
     */
    getComissoesStats = async (req, res) => {
        try {
            const { tenantId } = req.user;
            const { vendedorId } = req.query;
            const stats = await this.comissaoService.getStats(tenantId, vendedorId);
            res.json(stats);
        }
        catch (error) {
     ...
     */
    getRankingVendedores = async (req, res) => {
        try {
            const { tenantId } = req.user;
            const { mes, ano } = req.query;
            const ranking = await this.comissaoService.getRankingVendedores(tenantId, mes ? parseInt(mes) : undefined, ano ? parseInt(ano) : undefined);
            res.json(ranking);
        }
        catch (error) {
