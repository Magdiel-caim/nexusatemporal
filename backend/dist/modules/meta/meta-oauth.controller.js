...
class MetaOAuthController {
    ...
    async startOAuth(req, res) {
        try {
            ...
            await this.db.query(`INSERT INTO oauth_states (user_id, state, expires_at)
         VALUES ($1, $2, NOW() + INTERVAL '10 minutes')`, [userId, state]);
    ...
    async oauthCallback(req, res) {
        try {
            ...
            if (error) {
                ...
                const errorUrl = `${process.env.FRONTEND_URL}/integracoes?error=${encodeURIComponent(error)}`;
            ...
            const stateResult = await this.db.query(`SELECT user_id FROM oauth_states
         WHERE state = $1 AND expires_at > NOW()`, [state]);
            ...
            const instagramAccount = await this.metaService.getInstagramAccount(longLivedTokenData.access_token);
            // Encrypt token before storing
            const encryptedToken = this.metaService.encryptToken(longLivedTokenData.access_token);
            ...
            await this.db.query(`INSERT INTO meta_instagram_accounts (
    ...
    async listAccounts(req, res) {
        try {
         ...
         ORDER BY connected_at DESC`, [tenantId]);
            res.json({
                success: true,
                data: result.rows,
            });
    ...
    async disconnectAccount(req, res) {
        try {
            ...
            const result = await this.db.query(`UPDATE meta_instagram_accounts
         SET status = 'revoked', updated_at = NOW()
         WHERE id = $1 AND tenant_id = $2
         RETURNING instagram_username`, [id, tenantId]);
