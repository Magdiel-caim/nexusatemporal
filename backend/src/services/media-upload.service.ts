...
import { uploadFile } from '@/integrations/idrive/s3-client';
...
export class MediaUploadService {
  ...
  async uploadMediaFromUrl(
    ...
    try {
      logger.info(`[MediaUpload] Iniciando upload de mídia: ${mediaUrl}`);
      ...
      logger.info(`[MediaUpload] Arquivo baixado: ${fileSize} bytes, tipo: ${contentType}`);
      ...
      const fileUrl = await uploadFile(fileName, buffer, contentType, {
        source: 'whatsapp',
        originalUrl: mediaUrl,
        uploadedAt: new Date().toISOString(),
      });

      logger.info(`[MediaUpload] Upload concluído: ${fileUrl}`);
      ...
      logger.error('[MediaUpload] Erro ao fazer upload:', error.message || error);
   ...
   */
  getAttachmentType(mimeType: string): 'image' | 'video' | 'audio' | 'document' {
