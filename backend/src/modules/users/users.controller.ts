...
import { Pool } from 'pg';
import { UserRole, UserStatus } from '../auth/user.entity';
...
export class UsersController {
  ...
  constructor(private pool: Pool) {
    this.permissionsService = new PermissionsService(pool);
  }
  ...
  listUsers = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.pool.query(
        `SELECT id, email, name, phone, avatar, role, status, "tenantId", "emailVerified", "lastLoginAt", "createdAt", "updatedAt"
         FROM users
         ${whereClause}
  ...
  getUser = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.pool.query(
        `SELECT id, email, name, phone, avatar, role, status, "tenantId", "emailVerified", "lastLoginAt", permissions, preferences, "createdAt", "updatedAt"
         FROM users
         WHERE id = $1`,
        [id]
      );
  ...
  createUser = async (req: Request, res: Response) => {
    ...
    try {
      const user = req.user!;
      const { email, password, name, phone, role, status = UserStatus.ACTIVE } = req.body;
      ...
      const basicRoles = [UserRole.USER, UserRole.RECEPTIONIST, UserRole.PROFESSIONAL, UserRole.DOCTOR];
      ...
      const result = await client.query(
        `INSERT INTO users (email, password, name, phone, role, status, "tenantId", "passwordResetToken", "passwordResetExpires")
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         RETURNING id, email, name, phone, role, status, "tenantId", "createdAt"`,
        [email, hashedPassword, name, phone || null, role, status, tenantId, firstAccessToken, tokenExpires]
      );
  ...
  updateUser = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const basicRoles = [UserRole.USER, UserRole.RECEPTIONIST, UserRole.PROFESSIONAL, UserRole.DOCTOR];
      const privilegedRoles = [UserRole.OWNER, UserRole.ADMIN, UserRole.MANAGER, UserRole.SUPERADMIN, UserRole.SUPER_ADMIN];
      ...
      const result = await client.query(
        `UPDATE users
         SET ${updates.join(', ')}
         WHERE id = $${paramIndex}
         RETURNING id, email, name, phone, role, status, "tenantId", "updatedAt"`,
        params
      );
    ...
    try {
      ...
      await client.query(
        `UPDATE users
         ...
         WHERE id = $3`,
        [UserStatus.INACTIVE, deletedAt, id]
      );
   ...
   */
  getMyPermissions = async (req: Request, res: Response) => {
    try {
      const user = req.user!;
   ...
   */
  createUserFromPayment = async (req: Request, res: Response) => {
    const client = await this.pool.connect();
    try {
      const {
        email,
      ...
      const existingUser = await client.query(
        'SELECT id, email, name, role, status, "tenantId" FROM users WHERE email = $1',
        [email]
      );
      ...
      if (existingUser.rows.length > 0) {
        ...
        console.log('[External API] New user created', { userId, email, role: UserRole.OWNER });
   ...
   * Permissão: users.update ou users.update_basic
   */
  resendWelcomeEmail = async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const user = req.user!;
      ...
      // Buscar usuário
      const result = await this.pool.query(
        'SELECT id, email, name, "passwordResetToken", "passwordResetExpires" FROM users WHERE id = $1',
        [id]
      );
