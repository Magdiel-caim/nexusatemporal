...
export class PatientController {
  ...
  private imageService = new PatientImageService();
  private medicalRecordService = new PatientMedicalRecordService();
  ...
  uploadImage = async (req: Request, res: Response) => {
    try {
      ...
      // Upload para S3
      const uploadResult = await this.s3Service.uploadPatientImage(
        tenantId,
        patientId,
        file.buffer,
        file.originalname,
  ...
  getImages = async (req: Request, res: Response) => {
    try {
      ...
      const images = await this.imageService.findByPatient(
        patientId,
        tenantId,
        type as string
      );
   ...
   */
  getMedicalRecords = async (req: Request, res: Response) => {
    try {
      const { id: patientId } = req.params;
      const { tenantId } = req.user as any;
