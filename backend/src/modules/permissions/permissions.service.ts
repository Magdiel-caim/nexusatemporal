...
export class PermissionsService {
   ...
   */
  async hasPermission(userId: string, permission: Permission | string): Promise<boolean> {
    try {
   ...
   */
  async hasAllPermissions(userId: string, permissions: (Permission | string)[]): Promise<boolean> {
   ...
   */
  async hasAnyPermission(userId: string, permissions: (Permission | string)[]): Promise<boolean> {
    if (permissions.length === 0) return true;
  ...
  async hasRole(userId: string, roles: UserRole | UserRole[]): Promise<boolean> {
    try {
      ...
      const result = await this.pool.query<{ role: UserRole }>(
  ...
  async isSuperAdmin(userId: string): Promise<boolean> {
    return this.hasRole(userId, [UserRole.SUPERADMIN, UserRole.SUPER_ADMIN]);
  }
